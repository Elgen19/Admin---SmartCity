...
const FeedbackManagement = () => {
  ...
  const [ratingStats, setRatingStats] = useState({ highestRating: 0, lowestRating: 0, averageRating: 0 });
  ...
  const [error, setError] = useState(null);
   ...
   const fetchFeedbackSummary = async (tone, setSummaryState) => {
    try {
      const response = await fetch('http://localhost:5000/api/tone/analyze-feedback-tone', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Error fetching ${tone} summary: ${response.statusText}`);
      }
  ...
  const fetchCommentAnalysis = async () => {
    try {
        const requests = [
            axios.post('http://localhost:5000/api/type/analyze-feedback-type', { type: 'Bug Report' }),
            axios.post('http://localhost:5000/api/type/analyze-feedback-type', { type: 'Feature Suggestion' }),
            axios.post('http://localhost:5000/api/type/analyze-feedback-type', { type: 'General Feedback' })
        ];
        ...
        responses.forEach((response, index) => {
            if (response instanceof Error) {
                const errorMessage = response.response?.data?.error || 'Failed to fetch feedback summary.';
                switch (index) {
                    case 0:
                        setBugReportSummary(errorMessage.includes('No feedback comments available') ? 'No comments available for Bug Reports.' : errorMessage);
                        break;
                    case 1:
                        setFeatureSuggestionSummary(errorMessage.includes('No feedback comments available') ? 'No comments available for Feature Suggestions.' : errorMessage);
                        break;
                    case 2:
                        setGeneralFeedbackSummary(errorMessage.includes('No feedback comments available') ? 'No comments available for General Feedback.' : errorMessage);
                        break;
                    default:
                        break;
                }
...
useEffect(() => {
  ...
  const combinedDistribution = [
    { rating: 1, positiveCount: positiveDistribution[0].count, negativeCount: negativeDistribution[0].count },
    { rating: 2, positiveCount: positiveDistribution[1].count, negativeCount: negativeDistribution[1].count },
    { rating: 3, positiveCount: positiveDistribution[2].count, negativeCount: negativeDistribution[2].count },
    { rating: 4, positiveCount: positiveDistribution[3].count, negativeCount: negativeDistribution[3].count },
    { rating: 5, positiveCount: positiveDistribution[4].count, negativeCount: negativeDistribution[4].count },
  ];
      ...
      <div className="flex-1 flex flex-col items-start justify-start p-10 overflow-auto">
        ...
        {!showAllNegative && (
          <>
            ...
            <div
              className={`grid gap-6 ${showAllPositive ? "grid-cols-1" : "grid-cols-3"} w-full max-w-6xl mx-auto`}
            >
              {renderFeedbacks(positiveFeedbacks, showAllPositive).map(
                (feedback, index) => (
                  <div
                    ...
                    ${!showAllPositive ? "transition-transform transform hover:scale-105 shadow-lg" : ""} flex items-start`}
                  >
                    <img
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-lg font-medium text-gray-700 italic">"{feedback.comment}"</p>
                      <div className="flex items-center mt-2">
                        {Array.from({ length: 5 }, (_, i) => (
                          <svg
                            key={i}
                            className={`w-5 h-5 ${i < feedback.rating ? "text-yellow-500" : "text-gray-400"}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
        ...
        {!showAllPositive && (
          <>
            ...
            <div
              className={`grid gap-6 ${showAllNegative ? "grid-cols-1" : "grid-cols-3"} w-full max-w-6xl mx-auto`}
            >
              {renderFeedbacks(negativeFeedbacks, showAllNegative).map(
                (feedback, index) => (
                  <div
                    ...
                    ${!showAllNegative ? "transition-transform transform hover:scale-105 shadow-lg" : ""} flex items-start`}
                  >
                    <img
                      src={feedback.profilePic}
                      alt="Profile"
...
<div className="mt-8 flex justify-between">
  ...
  <div className="flex-1 mr-4">
    <h2 className="text-[#09d1e3] text-2xl font-bold">Feedback Ratings Distribution</h2>
    <BarChart
      width={600}
      height={300}
    ...
    >
      ...
      <Tooltip />
      <Legend />
      <Bar dataKey="positiveCount" fill="#33803d" name="Positive Feedback" />
      <Bar dataKey="negativeCount" fill="#b91c1c" name="Negative Feedback" />
    </BarChart>
  </div>
...
<div className="bg-white p-6 shadow-lg rounded-xl w-1/3 mx-auto my-8 transform transition-transform hover:scale-105 duration-300">
    <h3 className="text-[#09d1e3] text-2xl font-bold mb-4">Feedback Ratings Summary</h3>
    {loading ? (
        <p className="text-gray-500">Loading...</p>
    ) : (
        <div>
            <p className="text-gray-700">
                The highest rating received is <strong className="text-green-600">{ratingStats.highestRating}</strong>, attributed to <strong className="text-green-700">{ratingStats.highestCount}</strong> respondent(s).
            </p>
            <p className="mt-4 text-gray-700">
                Conversely, the lowest rating is <strong className="text-red-600">{ratingStats.lowestRating}</strong>, which reflects the opinions of <strong className="text-red-700">{ratingStats.lowestCount}</strong> individual(s).
            </p>
            <p className="mt-4 text-gray-700">
                The average rating across all feedback is <strong className="text-indigo-600">{ratingStats.averageRating}</strong>, providing insight into overall user satisfaction and experience.
            </p>
        </div>
    )}
</div>
...
<div className="p-6">
      <h2 className="text-[#09d1e3] text-3xl font-bold text-center mb-6">Comment Analysis</h2>
      ...
      <div className="feedback-card bg-green-100 border border-green-400 p-4 mb-6 rounded-lg shadow-lg transform transition-transform hover:scale-105 duration-300">
        <h4 className="text-xl font-semibold text-green-700 mb-2">Summary Of All Positive Comments</h4>
        <p className="text-green-600">{positiveFeedbackSummary || 'Loading positive feedback summary...'}</p>
      </div>
      ...
      <div className="feedback-card bg-red-100 border border-red-400 p-4 rounded-lg shadow-lg transform transition-transform hover:scale-105 duration-300">
        <h4 className="text-xl font-semibold text-red-700 mb-2">Summary Of All Negative Comments</h4>
        <p className="text-red-600">{negativeFeedbackSummary || 'Loading negative feedback summary...'}</p>
      </div>
    </div>


    <div className="p-6">
            <h2 className="text-[#09d1e3] text-3xl font-bold text-center mb-6">Comment Analysis by Type</h2>
            ...
            <div className="feedback-card bg-blue-100 border border-blue-400 p-4 mb-6 rounded-lg shadow-lg transform transition-transform hover:scale-105 duration-300">
                <h4 className="text-xl font-semibold text-blue-700 mb-2">Summary Of All Bug Reports</h4>
                <p className="text-blue-600">{bugReportSummary || 'Loading bug report summary...'}</p>
            </div>
            ...
            <div className="feedback-card bg-yellow-100 border border-yellow-400 p-4 mb-6 rounded-lg shadow-lg transform transition-transform hover:scale-105 duration-300">
                <h4 className="text-xl font-semibold text-yellow-700 mb-2">Summary Of All Feature Suggestions</h4>
                <p className="text-yellow-600">{featureSuggestionSummary || 'Loading feature suggestion summary...'}</p>
            </div>
            ...
            <div className="feedback-card bg-purple-100 border border-purple-400 p-4 rounded-lg shadow-lg transform transition-transform hover:scale-105 duration-300">
                <h4 className="text-xl font-semibold text-purple-700 mb-2">Summary Of All General Feedback</h4>
                <p className="text-purple-600">{generalFeedbackSummary || 'Loading general feedback summary...'}</p>
            </div>
        </div>
