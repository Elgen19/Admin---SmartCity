...
import { getDatabase, ref, push, onValue ,set, remove } from "firebase/database"; // Firebase import for Realtime DB
...
useEffect(() => {
  ...
  const handleSendClick = (content) => {
    ...
    // Only proceed if the channel is 'email' (case-insensitive check)
    if (normalizedChannel === "email") {
      fetch('http://localhost:5000/api/sender/send-content', {
        ...
        body: JSON.stringify({
          ...
          channel: content.channel,  // Send the exact channel from content
        }),
      })
        ...
        .then((data) => {
          if (data.message) {
            ...
            console.error('Failed to send content: No message returned from the server.');
          }
        })
        .catch((error) => {
      ...
      console.error('Invalid channel. Only email is supported for now.');
    }
  };
  ...
  <div className="flex h-screen bg-white font-nunito">
    ...
    <div className="flex-1 flex flex-col p-6 overflow-y-auto">
        <h1 className="text-2xl font-bold mb-2 text-[#09d1e3]">Content Management</h1>
        <p className="text-gray-700 mb-4">Create and manage content for your audience.</p>
        ...
        >
          <h2 className="text-xl font-semibold text-[#09d1e3]">Create Content</h2>
          <p className="text-gray-500 text-sm">Click here to create a new announcement, update, or reminder for your audience.</p>
        </div>
        ...
        {showAlert && (
          <div className="w-full max-w-md mx-auto">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
              ...
              <p className="text-gray-700">Your content has been successfully saved.</p>
              <button
...
<div className="mt-6">
  <h2 className="text-xl font-semibold text-[#09d1e3] mb-4">Saved Content</h2>
  <div className="grid grid-cols-1 gap-4">
    {contentList.length === 0 ? (
      <div className="bg-blue-100 shadow-md rounded-lg p-4 text-center">
        <h3 className="font-semibold text-lg">No Contents Available</h3>
        <p className="text-sm text-gray-500">There are currently no saved contents. Please add some content.</p>
      </div>
    ) : (
      contentList.slice(0, showAll ? contentList.length : 2).map((content) => (
        <div
        ...
        >
          <div>
            ...
            <div className="flex items-center text-sm text-gray-500 mt-2">
              ...
              <p>Posted by: {content.userName} ({content.userEmail})</p>
              <span className="material-icons text-base mx-2">access_time</span>
              <p>Timestamp: {new Date(content.timestamp).toLocaleString()}</p>
            </div>
          </div>
           {/* Icons Container */}
