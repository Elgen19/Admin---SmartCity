...
function SignUp() {
  ...
  const [verificationSent, setVerificationSent] = useState(false); // New state for verification status
    ...
    const verifyToken = async () => {
      ...
      try {
        const response = await axios.get(
          `https://smartcity-admin.vercel.app/api/verify-invite?token=${token}`
        );
...
const handleSignUp = async (e) => {
    ...
    setTimeout(async () => {
      try {
        const response = await axios.post('https://smartcity-admin.vercel.app/api/signup', {
          token,
          name: fullName,
          password,
        });

        // Indicate that verification email has been sent
